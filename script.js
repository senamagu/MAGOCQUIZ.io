      ...
      <div class="app-shell">
        <div class="screen">
          <div class="error">
            ...
            <p>${e.message}</p>
            <p>GitHub Pages等のHTTPサーバー環境で開いてください。</p>
          </div>
        </div>
      </div>`;
  }
...
function imgFor(c, type) {
  return c.images?.[type] || "";
}
...
function candidates() {
  return state.chars.filter(eligible);
}
...
function totalScore(c) {
  let total = 0;
  for (const [cat, w] of Object.entries(CAT_WEIGHT)) {
    total += categoryScore(c, cat) * w;
  }
  return total;
}
...
function categoryPenalty(q) {
  ...
  if (recent[recent.length - 1] === q.category) return 0.25;
  if (recent.includes(q.category)) return 0.5;
  return 1;
}
...
function featurePool(c) {
  ...
  for (const f of appearanceFields) {
    for (const t of (c.appearance?.[f] || [])) {
      out.push({ label: formatTag(t), score: Math.abs(state.scores[t] ?? 0) });
    }
  }
  ...
  if (c.alignment !== undefined) {
    const label = c.alignment >= 0 ? "善属性" : "悪属性";
    out.push({ label, score: Math.abs((state.scores.alignment ?? 0) * c.alignment) });
  }
  const seen = new Set();
...
function screenHTML() {
  if (state.screen === "genre") return genreHTML();
...
function genreHTML() {
  return `
    ...
    <section class="panel">
      <h1 class="title">ジャンルを選んでください</h1>
      <p class="muted">まずは見た目のジャンルから選ぼう。診断中の質問でさらに好みを引き出します。</p>
      <div class="genre-grid">
        ${[
          ...
          ["stick", "棒人間"],
          ["other", "その他"]
        ].map(([v, t]) => `<button class="choice" data-genre="${v}">${t}</button>`).join("")}
      </div>
    </section>`;
}
...
function questionHTML() {
  ...
  if (!state.current) state.current = q;
  ...
  return `
    ...
    <section class="panel">
      ...
      <div class="question-wrap">
        <div class="ghost">${bgImg ? `<img src="${esc(bgImg)}" alt="">` : ""}</div>
        <div class="question-content">
          <div class="question-area">
            <p class="question">${esc(templateText(q))}</p>
          </div>
          <div class="answers">
            <div class="answer-grid">
              ${ANSWERS.map(([v, t]) => `<button class="choice" data-answer="${v}">${t.replace('\n', '<br>')}</button>`).join("")}
            </div>
          </div>
        </div>
      </div>
      <div class="actions" style="margin-top:16px;">
        <button class="secondary" id="show-log">現在の候補を見る</button>
      </div>
    </section>`;
}

function resultHTML() {
  const r = state.resultRanks.length ? state.resultRanks : ranked();
  const top = r[0];
    ...
    <section class="panel">
      <div class="result-top">
        <div class="winner-art">
          ${top && imgFor(top, "fullbody") ? `<img src="${esc(imgFor(top, "fullbody"))}" alt="">` : ""}
        </div>
        <div class="result-info">
          <div class="muted">あなたにオススメのキャラ</div>
          <div class="result-reading">${esc(top?.reading || "")}</div>
          <h1 class="title" style="margin-bottom:0;">${esc(top?.name || "該当なし")}</h1>
          <div class="percent">${top ? Math.round(top.score) : 0}%</div>
          <p>${esc(top?.profile?.description || "")}</p>
          ${top ? `
            <div class="match-features">
              <strong>特に好みが一致した特徴</strong>
              <ul>${featurePool(top).map(x => `<li>${esc(x)}</li>`).join("")}</ul>
            </div>` : ""}
          <div class="actions">
            ${top ? `<button class="primary" id="detail">詳細を見る</button>` : ""}
            <button class="secondary" id="continue">診断を続ける</button>
            <button class="secondary" id="restart">もう一度診断する</button>
          </div>
        </div>
      </div>

      <h2 class="title" style="margin-top:36px;">その他のおすすめ</h2>
      <div class="rank-list">
        ${r.slice(1, 6).map(c => `
          <div class="rank clickable" data-rank-id="${esc(c.id)}">
            <img src="${esc(imgFor(c, "bust"))}" alt="">
            <div>
              <div class="result-reading">${esc(c.reading)}</div>
              <strong>${esc(c.name)}</strong>
            </div>
            <div class="p">${Math.round(c.score)}%</div>
          </div>`).join("")}
      </div>

      ${state.logs.length ? `
        <h2 class="title" style="margin-top:36px;">診断ログ</h2>
        <div class="log-list">
          ${state.logs.map((log, i) => `
            <div class="log-item">
              ${log.bg ? `<img class="log-bg" src="${esc(imgFor(log.bg, "bust") || imgFor(log.bg, "fullbody"))}" alt="">` : ""}
              <div class="log-text">
                <strong>${i + 1}回目（${log.questionCount}問時点）</strong>
                <div class="muted" style="margin-top:4px;">
                  上位: ${log.ranks.slice(0, 3).map(c => `${esc(c.name)} (${Math.round(c.score)}%)`).join(" / ")}
                </div>
              </div>
            </div>`).join("")}
        </div>` : ""}
...
function detailHTML() {
  const top = state.selectedCharacter || state.resultRanks[0] || ranked()[0];
  if (!top) return "";

  return `
    <div class="brand">キャラクター詳細</div>
    <section class="panel">
      <div class="detail">
        <div class="detail-art">
          ${imgFor(top, "fullbody") ? `<img src="${esc(imgFor(top, "fullbody"))}" alt="">` : ""}
        </div>
        <div class="profile">
          <div class="detail-reading">${esc(top.reading)}</div>
          <h1 class="title" style="margin-bottom:12px;">${esc(top.name)}</h1>
          
          <h2 class="title" style="font-size:20px;margin:18px 0 8px;">特に好みが一致した特徴</h2>
          <div class="chips">
            ${featurePool(top).map(x => `<span class="chip">${esc(x)}</span>`).join("")}
          </div>

          <dl>
            <dt>性別</dt><dd>${esc(top.profile.gender)}</dd>
            <dt>身長</dt><dd>${esc(top.profile.height)}</dd>
            <dt>年齢</dt><dd>${esc(top.profile.age)}</dd>
            <dt>好き</dt><dd>${esc(top.profile.likes.join("、"))}</dd>
            <dt>苦手</dt><dd>${esc(top.profile.dislikes.join("、"))}</dd>
            <dt>一人称</dt><dd>${esc(top.profile.firstPerson)}</dd>
            <dt>二人称</dt><dd>${esc(top.profile.secondPerson.join("、"))}</dd>
          </dl>

          <h2 class="title" style="font-size:20px;margin:24px 0 8px;">プロフィール</h2>
          <p>${esc(top.profile.description)}</p>

          <div class="actions">
            <button class="secondary" id="back-result">結果に戻る</button>
            ${top.profile.externalLink ? `<a class="primary" href="${esc(top.profile.externalLink)}" target="_blank" rel="noopener">さらに詳細</a>` : ""}
            <button class="secondary" id="restart">もう一度診断する</button>
          </div>
        </div>
      </div>
...
function bind() {
  ...
  if (detail) {
    detail.onclick = () => {
      state.selectedCharacter = state.resultRanks[0];
      state.screen = "detail";
      render();
    };
  }

  document.querySelectorAll("[data-rank-id]").forEach(b => {
    b.onclick = () => {
      state.selectedCharacter = state.resultRanks.find(c => c.id === b.dataset.rankId);
      state.screen = "detail";
      render();
    };
  });
  ...
  document.querySelectorAll("#restart").forEach(b => {
    b.onclick = () => {
      reset();
      state.screen = "genre";
      render();
  ...
  const back = document.getElementById("back-result");
  if (back) {
    back.onclick = () => {
      state.screen = "result";
      render();
