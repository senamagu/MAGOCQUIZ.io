...
function featurePool(c){
  const out=[];
  for(const f of appearanceFields) for(const t of (c.appearance?.[f]||[])) out.push({label:t,score:Math.abs(state.scores[t]??0)});
  for(const t of (c.personality?.tags||[])) out.push({label:t,score:Math.abs(state.scores[t]??0)});
  for(const t of (c.relationships||[])) out.push({label:t,score:Math.abs(state.scores[t]??0)});
  for(const t of (c.likesTags||[])) out.push({label:t,score:Math.abs(state.scores[t]??0)});
...
function detailHTML(){
  ...
  return `<div class="brand">キャラクター詳細</div><section class="panel"><div class="detail"><div class="detail-art">${imgFor(top,"fullbody")?`<img src="${esc(imgFor(top,"fullbody"))}" alt="">`:""}</div><div class="profile"><div class="detail-reading">${esc(top.reading)}</div><h1 class="title">${esc(top.name)}</h1><dl><dt>性別</dt><dd>${esc(top.profile.gender)}</dd><dt>身長</dt><dd>${esc(top.profile.height)}</dd><dt>年齢</dt><dd>${esc(top.profile.age)}</dd><dt>好き</dt><dd>${esc(top.profile.likes.join("、"))}</dd><dt>苦手</dt><dd>${esc(top.profile.dislikes.join("、"))}</dd><dt>一人称</dt><dd>${esc(top.profile.firstPerson)}</dd><dt>二人称</dt><dd>${esc(top.profile.secondPerson.join("、"))}</dd></dl><h2 class="title" style="margin-top:24px">特に好みが一致した特徴</h2><div class="chips">${featurePool(top).map(x=>`<span class="chip">${esc(x)}</span>`).join("")}</div><h2 class="title" style="margin-top:24px">プロフィール</h2><p>${esc(top.profile.description)}</p><div class="actions"><button class="secondary" id="back-result">結果に戻る</button>${top.profile.externalLink?`<a class="primary" href="${esc(top.profile.externalLink)}" target="_blank" rel="noopener">さらに詳細</a>`:""}<button class="secondary" id="restart">もう一度診断する</button></div></div></div></section>`;
}
function bind(){
